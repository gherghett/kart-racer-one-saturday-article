...
function startGame(mapData) {
  ...
  // Player physics
  const playerPhysics = new KartPhysics(track);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const botPhysics = new KartPhysics(track);
  ...
  function gameLoop(now) {
    ...
    if (!race.isFrozen()) {
      ...
      // Player physics
      playerPhysics.update(kart, input, dt, camera);

      // Bot physics
      for (const bot of bots) {
        bot.physics.update(bot.kart, bot.input, dt, camera);
      }
