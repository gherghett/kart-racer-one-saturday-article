...
export class KartPhysics {
  ...
  update(kart, input, dt, camera) {
    ...
    const triggerTap = ud.slideButton === 'z' ? input.hopXTap
                     ...
                     : false;
    // DEBUG
    if (input.hopZTap) console.log('Z PRESSED', { slideBtn: ud.slideButton, slideActive: ud.slideActive, grounded: ud.grounded });
    if (input.hopXTap) console.log('X PRESSED', { slideBtn: ud.slideButton, slideActive: ud.slideActive, grounded: ud.grounded });
    if (input.hopZRel) console.log('Z RELEASED', { slideBtn: ud.slideButton, slideActive: ud.slideActive, grounded: ud.grounded });
    if (input.hopXRel) console.log('X RELEASED', { slideBtn: ud.slideButton, slideActive: ud.slideActive, grounded: ud.grounded });
    ...
    if (!ud.grounded) {
      ...
      if (ud.slideActive) {
        ...
        const speedFactor = Math.min(1, (Math.abs(ud.speed) / s.maxSpeed) * 3);
        kart.rotation.y += input.steer * s.turnRate * speedFactor * dt;
      }
    }
    ...
    // --- Active boost: instant speed kick ---
    if (ud.boostTimer > 0) {
      const before = ud.speed;
      ud.speed = Math.max(ud.speed, maxSpd + ud.boostSpeed * 0.8);
      if (Math.abs(ud.speed - before) > 0.1) console.log('BOOST SPEED', { before: before.toFixed(1), after: ud.speed.toFixed(1), boostSpeed: ud.boostSpeed, timer: ud.boostTimer.toFixed(2) });
    }
    ...
    if (kart.position.y <= groundY + GROUND_SNAP) {
      if (gn.y < MAX_DRIVEABLE_SLOPE) {
        ...
        // Angle difference between momentum and facing → speed loss + sideways slide
        if (wasAirborne) {
