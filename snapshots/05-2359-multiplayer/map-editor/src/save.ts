import { type MapState, TerrainType, TERRAIN_COLORS, DEFAULT_TERRAIN_COLORS, createMap, getTerrainColor } from "./state.ts";
...
/** Generate color PNG as base64 data URL. Uses colorLayer where painted, terrain colors elsewhere. */
function colorToDataURL(map: MapState): string {
  const canvas = document.createElement("canvas");
  canvas.width = map.width;
  canvas.height = map.height;
  const ctx = canvas.getContext("2d")!;
  ...
  const colors = [
    ...
    const hex = getTerrainColor(map, t);
    return {
      r: parseInt(hex.slice(1, 3), 16),
      g: parseInt(hex.slice(3, 5), 16),
      b: parseInt(hex.slice(5, 7), 16),
  ...
  const cl = map.colorLayer;
  for (let i = 0; i < map.width * map.height; i++) {
    const j = i * 4;
    const clOff = i * 4;
    if (cl[clOff + 3]!) {
      img.data[j] = cl[clOff]!;
      img.data[j + 1] = cl[clOff + 1]!;
      img.data[j + 2] = cl[clOff + 2]!;
    } else {
      const t = map.terrain[i]!;
      const c = colors[t] ?? colors[0]!;
      img.data[j] = c.r;
  ...
  return JSON.stringify(
    {
      ...
      itemBoxes: map.itemBoxes,
      terrainColors: map.terrainColors,
      decalImages: map.decalImages.map((di) => ({ id: di.id, name: di.name })),
      decals: map.decals,
    },
    null,
    2,
...
export async function saveMap(map: MapState): Promise<boolean> {
  ...
  const color = colorToDataURL(map);
  const mapJson = mapToJSON(map);

  // Prepare decal images for saving
  const decalImages = map.decalImages.map((di) => ({
    id: di.id,
    name: di.name,
    dataUrl: di.dataUrl,
  }));
  ...
  const resp = await fetch("/api/save-map", {
    ...
    body: JSON.stringify({ name: map.name, terrain, heightmap, color, mapJson, decalImages }),
  });
...
export async function loadMap(name: string): Promise<MapState> {
  ...
  const meta = JSON.parse(payload.mapJson) as {
    ...
    itemBoxes?: MapState["itemBoxes"];
    terrainColors?: Record<number, string>;
    decalImages?: { id: string; name: string }[];
    decals?: MapState["decals"];
  };
  ...
  map.itemBoxes = meta.itemBoxes || [];
  if (meta.terrainColors) {
    map.terrainColors = { ...DEFAULT_TERRAIN_COLORS, ...meta.terrainColors };
  }
  ...
  // Load color layer from color.png — compare against terrain colors to detect painted pixels
  if (payload.color) {
    const { data: cData, width: cw } = await readImagePixels(payload.color);
    ...
    const w = Math.min(cw, meta.width);
    const h = Math.min(cData.length / (cw * 4), meta.height);
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const si = (y * cw + x) * 4;
        const cr = cData[si]!;
        ...
        // Check if this pixel differs from its terrain type's color
        const t = map.terrain[y * meta.width + x]!;
        const tc = terrainColorLookup[t] ?? terrainColorLookup[0]!;
        const diff = Math.abs(cr - tc.r) + Math.abs(cg - tc.g) + Math.abs(cb - tc.b);
        if (diff > 10) {
          const off = (y * meta.width + x) * 4;
          map.colorLayer[off] = cr;
          map.colorLayer[off + 1] = cg;
          map.colorLayer[off + 2] = cb;
  ...
  // Load decals
  if (meta.decals) {
    map.decals = meta.decals;
  }
  // Restore decal images — merge JSON metadata with actual image data from API
  if (payload.decalImages && payload.decalImages.length > 0) {
    map.decalImages = payload.decalImages;
  } else if (meta.decalImages) {
    // No images returned from API — create placeholders (images may be missing)
    map.decalImages = meta.decalImages.map((di) => ({ ...di, dataUrl: "" }));
  }
