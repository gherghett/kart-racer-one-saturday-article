...
export async function loadMap(mapId) {
  ...
  // Parse optional color image for per-pixel visual colors
  let colorData = null;
  if (colorImg) {
    const colorImgData = getImageData(colorImg);
    colorData = new Uint8Array(width * height * 3);
    const cw = Math.min(colorImg.width, width);
    const ch = Math.min(colorImg.height, height);
    for (let y = 0; y < ch; y++) {
      for (let x = 0; x < cw; x++) {
        const si = (y * colorImg.width + x) * 4;
        const di = (y * width + x) * 3;
        colorData[di] = colorImgData.data[si];
        colorData[di + 1] = colorImgData.data[si + 1];
        colorData[di + 2] = colorImgData.data[si + 2];
      }
    }
  }
  ...
  return {
    ...
    heightmap,
    colorData,
    terrainColors: terrainColors || null,
  };
}
